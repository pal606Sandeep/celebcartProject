 ...
 const router = useRouter()
  ...
  return (
    <div className='w-[95%] sm:w-[90%] md:w-[80%] mx-auto mt-8 mb-24 relative '>
      ...
      <Link href={"/"} className='absolute -top-2 left-0 flex items-center gap-2 text-green-600 hover:text-green-800 font-medium transition-all'>
        <ArrowLeft size={20}/>
      ...
      {cartData.length==0 ? (
        ...
        >
         ...
         <p className='text-gray-600 text-lg mb-6'>You Cart is empty. Add some Groceries to continue shopping!</p>
         <Link href={"/"} className='bg-green-600 text-white px-5 py-3 rounded-full hover:bg-green-700 transition-all inline-block font-medium'>
            Continue Shooping
         </Link>
        </motion.div>
      ):(
      <div className='grid grid-clos-1 lg:grid-clos-2 gap-8'>
        <div className='lg:col-span-2 space-y-5  '>
            <AnimatePresence >
                {cartData.map((item,idx)=>(
                  ...
                  >
                   <div className='relative w-28 h-28 sm:h-24 md:w-28 md:h-28 shrink-0 rounded-xl overflow-hidden bg-gray-50'>
                    <Image 
                    src={item.image} 
                    alt={item.name}
                    fill
                   ...
                   <div className='flex items-center justify-center sm:justify-end gap-3 mt-3 sm:mt-0 bg-gray-200 px-3 py-2 rounded-full'>
                     <button 
        ...
        >
         <h2 className='text-lg sm:text-xl fpnt-bold text-gray-800'>Order Summary</h2>
        ...
        <motion.button
        whileTap={{scale:0.95}}
        className='w-full mt-6 bg-green-600 text-white py-3 rounded-full hover:bg-green-700 transition-all font-semibold text-sm sn:text-base'
