...
useEffect(()=>{
  ...
  return (
    ...
    >
     ...
     {showLeft &&
     <button className='absolute left-0 top-1/2 -translate-y-1/2 z-10 bg-whitw shadow-lg hover:bg-green-100 rounded-full w-10 h-10 flex items-center justify-center mt-6' onClick={()=>scroll("left")}><ChevronLeft/></button>
     }
     ...
     {showRight && 
     <button className='absolute right-0 top-1/2 -translate-y-1/2 z-10 bg-whitw shadow-lg hover:bg-green-100 rounded-full w-10 h-10 flex items-center justify-center mt-6' onClick={()=>scroll("right")}><ChevronRight/></button>
     }
    </motion.div>
  )
}
