...
function Nav({user}:{user:IUser}) {
    ...
    const sideBar = menuOpen?createPortal(
      <AnimatePresence>
        ...
        exit={{x:100}}
        className='fixed top-0 left-0 h-full w-[75%] sm:w-[60%] z-9999 bg-linear-to-b from-green-800/90 via-green-700 to-green-900/90 backdrop-blur-xl border-r border-green-400/20 shadow-[0_0_50px_-10px_rgba(0,255,100,0.3)] flex flex-col p-6 text-white'
        >
        ...
        <div className='flex items-center gap-3 mt-3 p-3 rounded-xl bg-white/10 hover:bg-white/15 transition-all shadow-inner'>
         <div className='relative w-12 h-12 rounded-full overflow-hidden border-2 border-green-400/60 shadow-lg'>{user.image?<Image src={user.image} alt='user' fill className='object-cover rounded-full'/>:<User/>}
         </div> 
         <div>
        ...
        <div className='flex flex-col gap-3 font-medium mt-10'>
        <Link 
          href={"/admin/add-grocery"} 
          className='flex items-center gap-42 rounded-lg px- bg-white/10 hover:bg-white/20 hover:pl-4 transition-all'
        ><PlusCircle className='w-10 h-10'/>Add Grocery
        </Link>

        <Link 
         href={"/admin/view-grocery"}
         className='flex items-center gap-42 rounded-lg bg-white/10 hover:bg-white/20 hover:pl-4 transition-all'
         ><Boxes className='w-10 h-10'/>View Grocery
         </Link>

        <Link 
         href={"/admin/manage-orders"}
         className='flex items-center gap-42 rounded-lg bg-white/10 hover:bg-white/20 hover:pl-4 transition-all'
         ><ClipboardCheck className='w-10 h-10'/>Manage Order
        </Link>
        </div>
...
return (
 <div className='w-[95%] fixed top-4 left-1/2 -translate-x-1/2 bg-linear-to-r from-green-500 to-green-700 rounded-2xl shadow-black/30 flex justify-between items-center h-20 px-4 md:px-8 z-50'>
  ...
  {user.role=="user" &&
   ...
   <div className='flex items-center ap-3 md:gap-6 relative'>
    {user.role=="user" && 
    <>
    <div 
    ...
    <div className='hidden md:flex items-center gap-4'>
      ...
      href={"/admin/manage-orders"}
      className='flex items-center gap-2 bg-white text-green-700 font-semibold px-4 py-2 rounded-full hover:bg-green-100 transition-all'
      ><ClipboardCheck className='w-5 h-5'/>Manage Order
      </Link>
    </div>
    <div 
     ...
     <div className='relative' ref={profileDropDown}>
      ...
      {open && 
        ...
        <div className='flex items-center gap-3 py-2 border-b border-gray-100'>
            ...
            <div>
                ...
                <div className='taxt-xs taxt-gray-500 capitalize'>{user.role}</div>
            </div>
        </div>
      ...
      <AnimatePresence>
        {searchBarOpen 
         ...
         >
          <Search className='text-gray-500 w-5 h-5 mr-2'/>
          <form onSubmit={handelSearch} className='grow'>
            <input 
             onChange={(e)=>setSearch(e.target.value)}
             value={search}
            type="text" 
