...
function DeliveryBoyDashboard({earning}:{earning:number}) {
  ...
  const[verifyOtpLoading, setVerifySendOtpLoading] = useState(false)
  ...
  const[deliveryBoyLocation, setDeliveryBoyLocation] = useState<Ilocation>({latitude:0,longitude:0})
    ...
    const handelAcceptOrder = async (id:string)=>{
      try {
    ...
    if (!activeOrder && assignments.length===0) {
      ...
      return (
        ...
        <p className='nt-4 text-lg font-bold text-green-700'>{earning || 0} Earned today</p>
        <button 
          ...
          <div className='max-w-3xl mx-auto'>
           ...
           <div className='mt-6 bg-white rounded-xl border shadow p-6'>
            {!activeOrder.order.deliveryOtpVerification && !showOtpBox && (
              <button
              onClick={sendOtp}
              className='w-full py-4 bg-green-600 text-white rounded-lg text-center'>{sendotpLoading?<Loader size={16} className='animate-spin text-white text-center'/>:"Mark as Delivered"}</button>
            )}
            ...
            <div className='mt-4'>
              ...
              <button 
              onClick={verifyOtp}
              className='w-full mt-4 bg-blue-600 text-white py-3 rounded-lg text-center'>{verifyOtpLoading?<Loader size={16} className='animate-spin text-white text-center'/>:"Mark as Delivered"}</button>
  ...
  return (
    <div className='w-full min-h-screen bg-gray-50 p-4'>
        <div className='max-w-3xl mx-auto'>
          ...
          {assignments.filter(a => a.order !== null).map((a,idx)=>(
            <div 
            key={idx}
            ...
            >
              ...
              <div className='flex gap-3 mt-4'>
                <button 
                onClick={()=>handelAcceptOrder(a._id)}
                className='flex-1 bg-green-600 text-white py-2 rounded-lg'>Accept</button>
