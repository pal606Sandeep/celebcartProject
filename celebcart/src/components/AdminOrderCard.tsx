...
import { ChevronDown, ChevronUp, CreditCard, Home, Package, Phone, Truck, User, UserCheck } from 'lucide-react'
...
function AdminOrderCard({order}:{order:IOrder}) {
    ...
    useEffect(():any=>{
      ...
      return () => {
        socket?.off("order-status-update"); // Always cleanup listeners
      };
    },[order._id])
    ...
    >
      <div className='flex flex-col md:flex-row md:items-start md:justify-between gap-4'>
            ...
            <div className='mt-4 bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-center justify-between'>
              <div className='flex items-center gap-3 text-sm text-gray-700'>
                ...
                <div className='font-semibold text-gray-800'>
                  ...
                  <p className='text-xs text-gray-600'>📞 +91 {order.assignedDeliveryBoy.mobileno}</p>
                </div>
                </div> 
                <a 
                href={`tel:${order.assignedDeliveryBoy.mobileno}`}
        ...
        <div className='flex flex-col items-start md:items-end gap-2'>
            ...
            {currentStatus!=="delivered" && 
             <select 
             className='border border-gray-300 rounded-lg py-1 px-3 text-sm shadow-sm hover:border-green-400 transition focus:ring-2 focus:ring-green-500 outline-none'
             value={currentStatus}
            ...
            <div className='mt-3 space-y-3'>
              {order.items.map((item,index)=>(
                ...
                className='flex justify-between items-center bg-gray-50 rounded-xl px-3 py-2 hover:bg-gray-100 transition'>
                   ...
                   <div className='text-sm font-semibold text-gray-800'>₹{Number(item.price)*item.quantity}</div>
                </div>
              ))}
            </div>
           </motion.div>
